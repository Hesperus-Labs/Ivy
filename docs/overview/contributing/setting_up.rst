...
The Hesperus fork ships the transpiler as checked-in Python source. It does
not download private shared objects or mutate the checkout during installation.
Use :code:`uv sync --python 3.12 --extra all-cpu` for the complete local
framework matrix and see the published Pages site for the current primitive
coverage and Equinox migration contract.
